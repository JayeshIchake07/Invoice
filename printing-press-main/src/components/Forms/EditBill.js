...
useEffect(() => {
  const fetchInvoice = async () => {
    try {
      const response = await axios.get(`http://localhost:5000/api/invoice-bill/${bill_id}`);
      ...
      setInvoiceData({
        ...
        rupeesInWords: invoice.rupees_in_words || ''
      });
    } catch (err) {
  ...
  fetchInvoice();
}, [bill_id]);  // `bill_id` comes from useParams
    ...
    const onSubmit = async (e) => {
        ...
        const fullInvoiceData = { ...invoiceData, totalAmount, partyName: invoiceData.party_id };
